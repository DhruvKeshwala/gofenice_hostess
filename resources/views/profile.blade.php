...
<main class="w-100 mx-auto text-center" style="max-width: 800px;
    ...
    @endif
    <form id="formAccountSettings" method="POST" action="{{ route('profile.update',auth()->id()) }}" enctype="multipart/form-data">
        @csrf
        ...
        <label for="avatar" style="cursor:pointer">
            ...
            <div style="background-color: #e0e0e0;;border-radius: 50%;height: 200px;
                    width: 70%;margin: auto;opacity: 0.1;">
                <img src="{{ URL::asset('/upload/user/profile/' . Auth::user()->profilepic) }}"  style="width: 100%;
                border-radius: 47%;" alt="No Img">
            </div>
            @else
        ...
        <div class="form-floating">
            <input style="background-color: transparent;border: none;color:#ffffff" type="text" class="form-control"
                id="floatingFirstNameInput" placeholder="City / Country" name="city" value="@if(Auth::user()->city != null || Auth::user()->city != '')  {{Auth::user()->city}} @endif">
        ...
        <div class="form-floating">
            <fieldset style="background-color: transparent;border: none;color:#ffffff" >
                ...
                <div class="col-md-4">
                    <label><input type="radio" name="gender" value="Male" @if(Auth::user()->gender != null || Auth::user()->gender != '') @if(Auth::user()->gender == 'Male') checked @endif @endif> Male</label><br />
                </div>
                <div class="col-md-4">
                    <label><input type="radio" name="gender" value="Female" @if(Auth::user()->gender != null || Auth::user()->gender != '') @if(Auth::user()->gender == 'Female') checked @endif @endif> Female</label><br />
                </div>
                    </div><br>
            </fieldset>
